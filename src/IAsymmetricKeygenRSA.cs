...
namespace GlitchedPolygons.Services.Cryptography.Asymmetric
{
    ...
    public interface IAsymmetricKeygenRSA
    {
        ...
        /// If generation failed for some reason, <c>(null,null)</c> is returned.
        /// </summary>
        /// <param name="keySize">The desired RSA key size. Can be 512-bit, 1024-bit, 2048-bit or 4096-bit.</param>
        /// <returns>The key pair tuple where the first item is the public RSA key and the second one is the private key (both PEM-formatted strings). If key generation failed, both tuple items are <c>null</c>.</returns>
